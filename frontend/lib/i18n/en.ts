...
export const en: Dictionary = {
  ...
  steps: {
    ...
    alone: { name: "Seal a list", now: "You are alone on your side." },
    aloneFounder: "You are the only founder and there is not one builder to rank. A round needs {n} per side, and the minimum is per side, not the two added together. Send the link to whoever should be in it — they can join until it closes.",
    aloneBuilder: "You are the only builder and there is not one founder to rank. A round needs {n} per side, and the minimum is per side, not the two added together. Send the link to whoever should be in it — they can join until it closes.",
    copyInvite: "Copy the round link",
    copied: "Copied",
    ...
    signaturesWhyNone: "the session key signs the transaction and the enclave already knows you in this browser",
    signaturesWhyTwo: "one authorises a session key, one proves to the enclave who you are",
    signaturesWhySession: "only the session key is left to authorise; nothing after that",
    signaturesWhyEnclave: "only the enclave is left to convince; the session key is already authorised",
  ...
  play: {
    ...
    run: "Run",
    step: "One round",
    reroll: "Different tie-break",
    tiesIrrelevant: "On these lists the tie-break decides nothing: {n} different seeds give exactly the same pairing. One is only needed when a receiver has ranked neither of two proposers.",
    tiesMatter: "Here the tie-break decides: {d} of {n} seeds give a different pairing. Without verifiable randomness this would fall to registration order — whoever signed up first.",
    shuffle: "New lists",
    reset: "Reset",
